// To parse this data:
//
//   import { Convert, DezzerSearchResponse } from "./file";
//
//   const dezzerSearchResponse = Convert.toDezzerSearchResponse(json);
//
// These functions will throw an error if the JSON doesn't
// match the expected interface, even if the JSON is valid.

export interface DezzerSearchResponse {
    data:  InfoMusic[];
    total: number;
    next:  string;
}

export interface InfoMusic {
    id:                      number;
    readable:                boolean;
    title:                   string;
    title_short:             string;
...
export enum TitleVersion {
    DeMemoriasDeUnHombreEnPijama = "(De \"Memorias de un hombre en pijama\")",
    Empty = "",
    Fantástico = "(Fantástico)",
    IncapacidadMoralTransitoria = "(Incapacidad moral transitoria)",
}
...
export class Convert {
    public static toDezzerSearchResponse(json: string): DezzerSearchResponse {
        return cast(JSON.parse(json), r("DezzerSearchResponse"));
    }

    public static dezzerSearchResponseToJson(value: DezzerSearchResponse): string {
        return JSON.stringify(uncast(value, r("DezzerSearchResponse")), null, 2);
    }
}
...
const typeMap: any = {
    ...
    "Datum": o([
        ...
        { json: "explicit_content_lyrics", js: "explicit_content_lyrics", typ: 0 },
